...
class RoomEnd extends Room {  
  ...
  void display() {
    super.display();
    // display special text for when the player has found the dog treats
    if (inventory.hasItem("dog treats")) message = new Message("the dogs jumps you and steals a dog treat", 1000, width/2, height/3);
    else message = new Message("you did well, take a doggo", 10, width/2, height/3);
